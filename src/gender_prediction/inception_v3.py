...
inception_v3 = inception_v3(weights=Inception_V3_Weights.DEFAULT).to(device)

for param in inception_v3.parameters():
    param.requires_grad = False

num_ftrs = inception_v3.fc.in_features
inception_v3.fc = nn.Linear(num_ftrs, 2).to(device)
...
for epoch in range(epochs):
    ...
    train_epoch_loss, train_epoch_accuracy = train_binary(model=inception_v3, train_dataloader=train_loader, criterion=criterion, optimizer=optimizer)
    val_epoch_loss, val_epoch_accuracy = validate_binary(model=inception_v3, test_dataloader=test_loader, criterion=criterion)
...
labels, predicts = predict_gender(model=inception_v3, loader=test_loader, device=device)
