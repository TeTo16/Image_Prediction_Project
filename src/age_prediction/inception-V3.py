...
inceptionv3 = inception_v3(weights=Inception_V3_Weights.DEFAULT).to(device)

for param in inceptionv3.parameters():
    param.requires_grad = False

num_ftrs = inceptionv3.fc.in_features
inceptionv3.fc = nn.Linear(num_ftrs, 20).to(device)

criterion = nn.CrossEntropyLoss(reduction='sum')
optimizer = optim.Adam(inceptionv3.parameters(), lr=5e-4)
...
for epoch in range(epochs):
    ...
    train_epoch_loss, train_epoch_accuracy = train(model=inceptionv3, train_dataloader=train_loader, criterion=criterion, optimizer=optimizer)
    val_epoch_loss, val_epoch_accuracy = validate(model=inceptionv3, test_dataloader=test_loader, criterion=criterion)
...
labels, predicts = predict(model=inceptionv3, loader=test_loader, device=device)
